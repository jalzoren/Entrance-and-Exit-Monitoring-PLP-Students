...
const GenerateReportPdf = forwardRef(({ reportData = {}, filters = {} }, ref) => {
  ...
  const handleGeneratePDF = async () => {
    ...
    const opt = {
      ...
      filename: `eems_report_${new Date().toISOString().slice(0, 19).replace(/:/g, '-')}.pdf`,
  ...
  // ── FIXED: handle ALL possible field name variants from both old and new API ──
  const collegeDataArray = safeArray(collegeData);
  const processedCollegeData = collegeDataArray.map((dept, idx) => {
    // presentNow covers: new fixed API, old API field presenceNow, and other variants
    const presentNow =
      dept.presentNow      ??  // set by our fixed Analytics.jsx handleApplyFilters
      dept.presenceNow     ??  // raw from /api/analytics/departments
      dept.currentStudents ??
      dept.student_count   ??
      0;

    // totalEnrolled covers all variants
    const totalEnrolled =
      dept.totalEnrolled   ??  // set by our fixed Analytics.jsx
      dept.totalStudents   ??  // raw from /api/analytics/departments
      dept.enrolled_count  ??
      0;
    ...
    const pctCampus =
      dept.percentageOfCampus != null
        ? parseFloat(dept.percentageOfCampus)
    ...
    return {
      ...
      percentageOfCampus: pctCampus,
    };
  }).sort((a, b) => b.presentNow - a.presentNow);

  // Recalculate % of campus dynamically from actual data
  ...
  // ── FIXED green box numbers: prefer currentOnCampus from props, else sum from dept data ──
  ...
  const actionType    = filters?.actionType || 'both';
  const showEntryPage = actionType === 'both' || actionType === 'entry';
  const showExitPage  = actionType === 'both' || actionType === 'exit';

  // ── Stable page numbers ──
  const pageCount = 3 + (showEntryPage ? 1 : 0) + (showExitPage ? 1 : 0);
  const p1 = 1;
  const p2 = 2;
  const p3 = 3;
  const p4 = showEntryPage ? 4 : null;
  const p5 = showExitPage  ? (showEntryPage ? 5 : 4) : null;
  ...
  return (
    <div className="pdf-container">
      <div ref={reportRef} className="pdf-report landscape">
        ...
        <div className="pdf-page">
          <div className="pdf-header">
            ...
            <div className="pdf-stats-section">
              {/* ── FIXED GREEN BOX: uses dynamic displayOnCampus / displayTotalEnrolled ── */}
        ...
        {/* ══ PAGE 2: DAILY TRAFFIC TREND & VISITOR STATS ══ */}
        <div className="pdf-page">
          ...
          <div className="pdf-section-spacing">
            ...
            <p style={{ fontSize: '12px', color: '#666', marginBottom: '12px' }}>
              <strong>Date Range:</strong> {formatDateRange()}
              {filters?.collegeDepartment && (
                <>&nbsp;|&nbsp;<strong>Department:</strong> {filters.collegeDepartment}</>
              )}
            </p>
            ...
            <div style={{ overflowX: 'auto' }}>
              <table className="pdf-table">
                <thead>
                  <tr>
                    <th style={thGreen}>Date/Day</th>
                    <th style={thGreen}>Entrances</th>
                    <th style={thGreen}>Exits</th>
                    <th style={thGreen}>Total</th>
                  </tr>
                </thead>
                <tbody>
                  {processedTrafficData.length > 0 ? (
                    processedTrafficData.map((day, i) => (
                      <tr key={i}>
                        <td style={{ padding: '6px', textAlign: 'center' }}>{day.date}</td>
                        <td style={{ padding: '6px', textAlign: 'center', color: '#2E7D32' }}>{day.entrance.toLocaleString()}</td>
                        <td style={{ padding: '6px', textAlign: 'center', color: '#D99201' }}>{day.exit.toLocaleString()}</td>
                        <td style={{ padding: '6px', textAlign: 'center' }}>{day.total.toLocaleString()}</td>
                      </tr>
                    ))
                  ) : (
                    <tr>
                ...
                {processedTrafficData.length > 0 && (
                  <tfoot>
                    <tr style={{ backgroundColor: '#e8f5e9', fontWeight: 'bold' }}>
                      ...
                      <td style={{ padding: '8px', textAlign: 'center', color: '#2E7D32' }}>
                        {processedTrafficData.reduce((s, d) => s + d.entrance, 0).toLocaleString()}
                      </td>
                      <td style={{ padding: '8px', textAlign: 'center', color: '#D99201' }}>
                        {processedTrafficData.reduce((s, d) => s + d.exit, 0).toLocaleString()}
                      </td>
                      <td style={{ padding: '8px', textAlign: 'center' }}>
                        {processedTrafficData.reduce((s, d) => s + d.total, 0).toLocaleString()}
                      </td>
                    </tr>
                  </tfoot>
                )}
            ...
            <div className="pdf-insights-box">
              <div><span className="pdf-insight-text">Highest traffic: </span><span style={{ fontSize: '12px' }}>{highestTraffic}</span></div>
              <div><span className="pdf-insight-text">Lowest traffic: </span><span style={{ fontSize: '12px' }}>{lowestTraffic}</span></div>
              <div><span className="pdf-insight-text">Peak Hour: </span><span style={{ fontSize: '12px' }}>{getPeakHourDisplay()}</span></div>
            </div>

            {(visitorEntries > 0 || visitorExits > 0) && (
              <>
                <h3 className="pdf-section-title" style={{ marginTop: '20px' }}>Visitor Entry and Exit</h3>
                <div style={{ display: 'flex', gap: '20px', marginTop: '10px' }}>
                  <div style={{ flex: 1, textAlign: 'center', padding: '20px', backgroundColor: '#e8f5e9', borderRadius: '8px' }}>
                    <div style={{ fontSize: '32px', fontWeight: 'bold', color: '#2E7D32' }}>{visitorEntries.toLocaleString()}</div>
                    <div style={{ fontSize: '14px', color: '#666' }}>Visitor Entries</div>
                  </div>
                  <div style={{ flex: 1, textAlign: 'center', padding: '20px', backgroundColor: '#fff3e0', borderRadius: '8px' }}>
                    <div style={{ fontSize: '32px', fontWeight: 'bold', color: '#D99201' }}>{visitorExits.toLocaleString()}</div>
                    <div style={{ fontSize: '14px', color: '#666' }}>Visitor Exits</div>
                  </div>
                </div>
              </>
            )}

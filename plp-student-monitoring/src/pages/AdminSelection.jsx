...
function AdminSelection(){
  ...
  const methods = [
    { key: '1', action: () => navigate('/login'), icon: <FaUserCog />, label: "Super Admin", description: "Full system control and configuration", color: 'super' },
    { key: '2', action: () => navigate('/login'), icon: <FaUserFriends />, label: "EEM Admin", description: "Entrance & Exit Monitoring System", color: 'eem' },
    { key: '3', action: () => navigate('/login'), icon: <FaUserTie />, label: "EAM Admin", description: "Employee Attendance Monitoring", color: 'eam' },
  ];
        ...
        <div className="selections">
          ...
          <div className="selection-options">
            {methods.map(({ key, action, icon, label, description }) => (
              <div
                className={`selection-card admin-card ${color} ${activeKey === key ? 'active-key' : ''}`}
                key={label}
                onClick={action}
              >

...
function ExitMethodSelection() {
  ...
  const methods = [
    { key: 'f', action: () => navigate('/facerecog-exit'), icon: <LuScanFace />,   label: "Face Scan",    description: "Verify via camera scan." },
    ...
    <div className="selection-container">
      <div className="selection-overlay">
        ...
        <div className="selections">
          <div className="mode-badge exit-badge">EXIT</div>
          <p className="selections-prompt">
            Please select your preferred identification method for{' '}
            <span className="emphasize-exit">exit</span>. <br />
            Press <span className="emphasize-keys">F, Q, or M</span> to proceed instantly.
          </p>
          <div className="selection-options" style={{ gridTemplateColumns: 'repeat(3, minmax(180px, 220px))' }}>
      ...
      {activeModal === 'manual' && <ManualInputModal mode="EXIT" onClose={() => setActiveModal(null)} />}
      {activeModal === 'qr'     && <QRScanModal      mode="EXIT" onClose={() => setActiveModal(null)} />}
    </div>
  );
}

export default ExitMethodSelection;
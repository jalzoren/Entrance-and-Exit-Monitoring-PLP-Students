...
const AnalyticsService = {
  ...
  async fetchStudentPairing(filters = {}) {
  try {
    const params = new URLSearchParams();
    if (filters.from) params.set('from', filters.from);
    if (filters.to) params.set('to', filters.to);
    if (filters.dept) params.set('dept', filters.dept);
    const res = await fetch(`/api/analytics/student-pairing?${params}`);
    if (!res.ok) throw new Error(`student-pairing: HTTP ${res.status}`);
    return await res.json();
  } catch (err) {
    console.error('[AnalyticsService.fetchStudentPairing] FAILED:', err.message);
...
function Analytics() {
  ...
  const loadAll = useCallback(async (days) => {
    ...
    try {
      const [
        ...
        AnalyticsService.fetchVisitorLogs(),
      ]);
      ...
      const formattedDepts = deptData.map((dept) => ({
        ...
        collegeName:
          ...
          "Unknown",
        presenceNow:
          dept.presenceNow ?? dept.currentStudents ?? dept.presentNow ?? 0,
        totalStudents:
          dept.totalStudents ?? dept.enrolled ?? dept.total_enrolled ?? 0,
        percentage:
          dept.percentage ||
          ((dept.presenceNow / (dept.totalStudents || 1)) * 100).toFixed(1) ||
          "0",
      }));
      ...
      if (visitorStats && Array.isArray(visitorStats)) {
        const entryItem = visitorStats.find(
          (v) => v.name === "Entry" || v.name === "ENTRY",
        );
        const exitItem = visitorStats.find(
          (v) => v.name === "Exit" || v.name === "EXIT",
        );

        if (entryItem && exitItem) {
          entries = entryItem.value || 0;
          exits = exitItem.value || 0;
        } else {
          entries = visitorStats.filter(
            (v) => (v.action || "").toLowerCase() === "entry",
          ).length;
          exits = visitorStats.filter(
            (v) => (v.action || "").toLowerCase() === "exit",
          ).length;
        }
      ...
      setError(
        "Failed to load analytics data. Please check your server connection.",
      );
    } finally {
      setIsLoading(false);
    }
  ...
  const currentCollegeData = collegeData.slice(
    indexOfFirst,
    ...
    <div className="analytics-page">
      ...
      <div className="analytics-container">
        ...
        <div className="metrics-row">
          ...
          <div className="metric-card">
            <div className="metric-value">
              {metrics.currentStudentsInside.toLocaleString()}
            </div>
        ...
        {error && (
          <div className="error-state">
            <p>{error}</p>
            <button onClick={() => loadAll(daysMap[timeRange] ?? 7)}>
              Retry
            </button>
          </div>
        )}
                  ...
                  {insights && (
                    <div className="traffic-insights-container">
                      <div className="insights">
                        ...
                        <ul>
                          <li>
                            <strong>Highest traffic:</strong>{" "}
                            {insights.highest?.date} (
                            {insights.highest?.entrance?.toLocaleString() || 0}{" "}
                            entries)
                          </li>
                          <li>
                            <strong>Lowest traffic:</strong>{" "}
                            {insights.lowest?.date} (
                            {insights.lowest?.entrance?.toLocaleString() || 0}{" "}
                            entries)
                          </li>
                        </ul>
                      </div>
  ...
  <section className="chart-section">
    <div className="section-header">
      ...
      <button
        className="info-btn"
        title="Shows how students authenticated at the gate."
      >
        ℹ
      </button>
    </div>
  ...
  <div className="table-container small-table" style={{ marginTop: '20px' }}>
    <table className="analytics-table small-table">
      <thead>
        <tr>
          <th>No.</th>
      ...
      <tbody>
        {visitorLogs.slice(0, 5).map((visitor, i) => (
          <tr key={visitor.visitor_id || i}>
            <td>{i + 1}</td>
            <td>{visitor.full_name || 'Unknown'}</td>
            <td>{visitor.reason || 'Not Specified'}</td>
            <td>
              <span style={{
                padding: '2px 8px',
                borderRadius: '4px',
                fontSize: '11px',
                fontWeight: 'bold',
                backgroundColor: visitor.action === 'ENTRY' ? '#e8f5e9' : '#ffebee',
                color: visitor.action === 'ENTRY' ? '#2e7d32' : '#c62828'
              }}>
                {visitor.action === 'ENTRY' ? 'IN' : 'OUT'}
              </span>
            </td>
            <td>{new Date(visitor.log_time).toLocaleTimeString()}</td>
          </tr>
        ))}
      </tbody>
    </table>
      ...
      <p className="no-data-msg">No visitor data available.</p>
            ...
            <section className="chart-section">
              <div className="section-header">
                ...
                <button
                  className="info-btn"
                  title="Students currently on campus by department."
                >
                  ℹ
                </button>
              </div>
              {collegeData.length > 0 ? (
                <>
                  ...
                  <div className="campus-summary">
                    <p>
                      ...
                      {collegeData
                        .reduce((s, d) => s + (d.presenceNow || 0), 0)
                        .toLocaleString()}{" "}
                      ...
                      <tbody>
                        {currentCollegeData.map((college, i) => (
                          <tr key={college.fullCollegeName}>
                            ...
                            <td title={college.fullCollegeName}>
                              {college.fullCollegeName}
                            </td>
                            <td>
                              {(college.presenceNow || 0).toLocaleString()}
                            </td>
                            <td>
                              {(college.totalStudents || 0).toLocaleString()}
                            </td>
                ...
                <p className="no-data-msg">
                  No department data available. Students need to be on campus.
                </p>
              )}
            </section>
          </>
...
function CollegeDistributionChart({ data }) {
  ...
  return (
    <div
      className="chart-container college-chart"
